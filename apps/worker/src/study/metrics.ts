 ...
 * output.
 *
 * DEDUPLICATION IS NOT OPTIONAL. Pilot 01 wrote two `agent_decision` rows per
 * offset, which doubles the denominator of the silence rate and halves it
 * again depending on which one you keep. The container numbers the MOMENT on
 * every decision (`opportunitySeq`) and counts the completions within it
 * (`attempt`), so a moment is one distinct `opportunitySeq` and what it became
 * is its highest `attempt` — the rule the schema states and `study:export`
 * already carries. A drive recorded before the container numbered them has to
 * be deduplicated on `offsetMs` instead, which is what `momentsOf` does.
 *
 * The extra completions are counted separately, as `doubleDispatched`, because
 * how often one moment runs two of them is a finding of its own.
 */
import {
  agentDecision,
  agentTurn,
...
/**
 * One row per MOMENT, from the rows the container wrote.
 *
 * Pipecat runs inference more than once inside a user turn, so several
 * decisions can share one moment. What the moment BECAME is its last attempt —
 * the completion that finally spoke, or the last one that declined — which is
 * the rule `agentDecision` states in the schema and `study:export` orders its
 * rows by.
 *
 * Two shapes of input, and the older one is the reason this is a function
 * rather than a filter. A drive recorded before the container numbered its
 * moments has `opportunitySeq` null on every row, and the only thing left to
 * group on is `offsetMs` — coarser, and right often enough that dropping those
 * drives would be worse. Rows arrive ordered by attempt, so the last one seen
 * for a key is the one to keep.
 */
function momentsOf(rows: readonly DecisionRow[]): DecisionRow[] {
  const byMoment = new Map<string, DecisionRow>();
  for (const row of rows) {
    const key = row.opportunitySeq === null ? `t${row.offsetMs}` : `o${row.opportunitySeq}`;
    const held = byMoment.get(key);
    if (held === undefined || row.attempt >= held.attempt) byMoment.set(key, row);
  }
...
export interface Tier1 {
  ...
  turns: number;
  /**
   * Liveness keep-alives — the "still looking" phrase spoken while a tool runs.
   *
   * `backchannel`, which is how the container files them. The sentence that
   * OPENS a lookup is the model's own and is filed as a reply, deliberately:
   * it answers the person, in the language of the conversation, and only the
   * keep-alive behind it is the system talking about itself.
   */
  fillers: number;
...
export interface Tier2 {
  ...
  /**
   * Moments where the agent tried to meet an answer with silence. TARGET: 0.
   *
   * The single number Pilot 01 would have failed on: the agent asked a yes/no
   * question, heard "Ja.", and replied `<silence>`.
   *
   * `AnswerGuard` now refuses that completion and re-runs the turn, and it
   * deliberately writes no decision for the refusal — their words were ONE
   * moment to speak, and the re-run is what that moment became. Which leaves
   * the guard invisible unless the moment is marked, so it is:
   * `agent_decision.forced_answer`. This counts those marks.
   *
   * A non-zero value does not mean the person was left in silence — the guard
   * caught it, and either the re-run spoke or the acknowledgement did. It
   * means the model still tries, and somebody waited an extra completion for
...
export async function participantMetrics(
  ...
  if (sessionIds.length > 0) {
    const [turns, decisions, utterances, directives, invocations, ops, responses] =
      await Promise.all([
        ...
        db
          ...
          .orderBy(asc(agentDecision.captureSessionId), asc(agentDecision.attempt)),
        db
          .select({
            id: utterance.id,
    ...
    for (const session of sessions) {
      ...
      const spoken = sessionTurns.filter((t) => t.kind !== "backchannel");
      ...
      const counted = momentsOf(sessionDecisions);
      ...
      const tier1: Tier1 = {
        ...
        endsMeasured: sessionTurns.filter((t) => t.endOffsetMeasured).length,
        opportunities: counted.length,
        silent,
        silentShare: share(silent, counted.length),
        // Completions a moment ran beyond the first. Not noise: how often one
        // moment needs two is what the `attempt` column was added to show.
      ...
      const tier2: Tier2 = {
        ...
        // Per MOMENT, because that is what the mark is on. The refused
        // completion wrote nothing; the decision this counts is the one the
        // re-run produced, carrying `forcedAnswer`.
        unansweredAnswers: counted.filter((d) => d.forcedAnswer).length,
      };
